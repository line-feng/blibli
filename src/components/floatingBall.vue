<template>
	<transition>
		<div ref="dragIcon" class="dragIcon"  @touchmove.prevent.stop="handleTouchMove($event)"
		 @touchend.stop="handleTouchEnd" :style="{left: left + 'px',top: top + 'px',width: itemWidth + 'px',height: itemHeight + 'px'}"
		  v-if="isShow">
		  <span @touchstart.stop="handleTouchStart">
			  F
		  </span>
		   <slot></slot>
		</div>
...
<script>
	...
	export default {
		...
		created() {
			...
			this.top = (this.clientH - this.clientH / 7) 
		},
		mounted() {
